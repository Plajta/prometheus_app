...
function computeStatuses(
    ...
    const upcoming = events.filter((e) => {
        if (skipPresentTime && e.time.getTime() === skipPresentTime.getTime()) return false;
        return e.time >= now;
    });
...
function SlotCell({
    ...
    const [confirmedChoice, setConfirmedChoice] = useState<"taken" | "skipped" | null>(null);
    const pulseAnim = useRef(new Animated.Value(1)).current;

    const displayKey: StyleKey =
        confirmedChoice === "taken" ? "past"
        : confirmedChoice === "skipped" ? "overdue"
        : status === "past" && !slot.taken ? "overdue"
        : status;

    const s = (isDark ? STATUS_STYLE_DARK : STATUS_STYLE)[displayKey];
    const shouldBlink = isInWindow && confirmedChoice === null;
    const canInteract = (status === "past" || (status === "present" && isInWindow)) && confirmedChoice === null;
    ...
    const handleChoose = (taken: boolean) => {
        ...
        setSplit(false);
        onSetTaken(taken);
        if (status === "present" && isInWindow) {
            setConfirmedChoice(taken ? "taken" : "skipped");
            onConfirmedPresent?.();
            if (taken) {
                const stored = getStoredSchedule();
                if (stored) {
                    confirmSchedule(stored.schedule_id).catch(console.error);
                }
            }
        }
    };
    ...
    return (
        <View className="flex-1 flex-row gap-0.5">
            {split ? (
                ...
                <Animated.View style={{ flex: 1, opacity: shouldBlink ? pulseAnim : 1 }}>
                    <Pressable
                    ...
                    >
                        ...
                        {!shouldBlink && status === "present" && countdown ? (
                            <View className="absolute top-1 right-1.5">
...
export default function DeviceScreen() {
    ...
    const [skipPresentTime, setSkipPresentTime] = useState<Date | null>(null);
    ...
    const handleConfirmedPresent = useCallback(() => {
        const newSkip = slotStatuses.presentEventTime;
        setSkipPresentTime(newSkip);
        const { mh, mm, eh, em } = alarmTimes.current;
        setSlotStatuses(computeStatuses(mh, mm, eh, em, newSkip));
    }, [slotStatuses.presentEventTime]);
    ...
    const loadSettings = useCallback(() => {
        ...
        if (s) {
            ...
            setSlotStatuses(computeStatuses(mh, mm, eh, em, null));
            setSkipPresentTime(null);
        }
    }, []);
                        ...
                        {slotsA.map((slotA, i) => (
                            <View key={i} className="flex-1 flex-row gap-1.5">
                                <SlotCell
                                    ...
                                    onConfirmedPresent={handleConfirmedPresent}
                                />
                                <SlotCell
                                    slot={slotsB[i]}
